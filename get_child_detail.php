...
if (isset($_POST['id']) && !empty($_POST['id'])) {
    ...
    // Prepare SQL query to fetch child details with related data
    $sql = "SELECT e.id, e.nom, e.prenom, e.sexe, e.date_naissance, e.adresse, e.idniveau, e.idclasse, e.photo, e.idetablissement, 
                   n.niveau AS niveau_nom, 
...
if (isset($_POST['id']) && !empty($_POST['id'])) {
    ...
    // Prepare SQL query to fetch child details with related data
    $sql = "SELECT e.id, e.nom, e.prenom, e.sexe, e.date_naissance, e.adresse, e.idniveau, e.idclasse, e.photo, e.idetablissement, " .
           "t.nomprenom AS tuteur_nomprenom, t.email AS tuteur_email, t.telephone AS tuteur_telephone, " .
           "c.classe AS classe_nom, n.niveau AS niveau_nom, et.nom AS etablissement_nom " .
           "FROM talimnet_eleves e " .
           "LEFT JOIN talimnet_tuteur_eleves te ON e.id = te.ideleve " .
           "LEFT JOIN talimnet_tuteur t ON te.idtuteur = t.id " .
           "LEFT JOIN talimnet_classes c ON e.idclasse = c.id " .
