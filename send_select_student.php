...
if (!isset($data['idUser']) || !isset($data['selectedStudentId']) || !isset($data['idetablissement']) || !isset($data['message'])) {
...
// Fetch the parent ID for the selected student
$sqlParent = "SELECT id FROM talimnet_tuteur WHERE ideleve = ?";
$stmtParent = $conn->prepare($sqlParent);
if (!$stmtParent) {
    die(json_encode(["error" => "Error preparing parent select statement: " . $conn->error]));
}
$stmtParent->bind_param("i", $selectedStudentId);
$stmtParent->execute();
$resultParent = $stmtParent->get_result();

if ($resultParent->num_rows > 0) {
    $rowParent = $resultParent->fetch_assoc();
    $selectedParentId = $rowParent['id'];
} else {
    die(json_encode(["error" => "Error: No parent found for the selected student."]));
}
